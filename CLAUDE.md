...
- **Railway 로그 열람 (2026-09-27 정정)**: ① **REMOVED 상태의 배포도 로그를 볼 수 있다** —
  배포가 교체돼 목록에서 REMOVED로 표시돼도 그 배포의 로그는 그대로 열린다.
  "REMOVED라 못 본다"고 지레 포기하지 말 것. ② **로그 검색창에 대괄호·따옴표를
  넣으면 검색이 실패한다** — 이 레포 로그 태그가 전부 `[disk-cache]`/`[mcap]`/`[TIMING]`
  꼴이라 그대로 붙여넣기 쉬운데, 대괄호를 빼고 `disk-cache`·`mcap`·`TIMING`으로
  검색해야 결과가 나온다(따옴표로 감싸는 것도 실패).
...
- **새 CONFIG 조건·임계값(scanner.py의 `*_CONFIG` dict, 새 `analyze_*` 게이트 수치 등)을 도입할 때는 출처를 반드시 기록한다.** 출처 유형별 기록 방식:
  1. **웹 검색** → 실제 URL/링크(`docs/ud_volume_ratio_investigation.md`의 "웹 검색으로 확인한 IBD/MarketSmith 표준 정의...와 정확히 일치"가 올바른 선례).
  2. **사용자 지시** → 사용자가 실제로 뭐라고 말했는지 원문(또는 원문에 가까운 요약) 인용. **"사용자 지시"라고만 쓰지 말 것** — 이 한 단어로는 "이 작업 자체를 시켰다"인지 "이 구체적 숫자를 지정했다"인지 나중에 구분이 안 된다.
  3. **측정 결과로 도출** → `scripts/measurements/`의 근거 스크립트 경로(이미 이 레포의 관례).
  4. **AI(나) 판단으로 임의 설정** → 그렇다는 사실 자체를 명시(예: "웹 검색·백테스트 없이 어림값으로 설정, 재검토 필요"). 얼버무리지 않는다.
  근거: 2026-09-09 감사에서 `JONGGA_CONFIG`(종가베팅 5조건 — 거래대금상위100/+3%양봉/-2%윗꼬리/거래량2배/52주고점-15%이내)의 도입 커밋(`f2bc2d5`, v5.97)과 그 직전 측정 커밋(`ba66851`) 어디에도 이 구체적 숫자들의 최초 출처가 없었다 — docstring의 "(사용자 지시)"는 "백테스트 작업을 시켰다"는 뜻인지 "이 숫자를 지정했다"는 뜻인지 구분이 안 됐고, `docs/kr_jongga_betting_backtest.md`의 "실전 고수 조건"이라는 단서도 인용·링크 없이 그냥 문장으로만 남아 있었다(같은 레포 안에 `ud_volume_ratio_investigation.md`라는 올바른 선례가 있었는데도). 사후 성과(z=3.54)로 조건 자체는 검증됐지만, "왜 이 숫자였는가"는 영구히 재구성 불가능해졌다 — `docs/kr_jongga_betting_backtest.md` 상단 콜아웃에 이 감사 결과를 기록해둠.
...
- **데이터 소스(naver siseJson) 오염 방어(`app._filter_invalid_bars()`, v5.242)를 측정 스크립트에서 쓰려면 `harness.clean_at_checkpoint()`를 **체크포인트까지 자른 df에** 적용할 것 — fetch 시점 `_downcast()`에 넣으면 룩어헤드다.** 배경: 2026-09-10 종가베팅 측정(`scripts/measurements/2026-09-10_jongga_exit_timing.py`) 중 naver siseJson API가 무거래일에 OHLC를 0으로, Close만 전일가로 이월해서 반환하는 오염을 발견했다(예: `201490.KQ`, `007610.KS`는 2022-03~2026-04 약 4년치가 이 패턴 — 장기 거래정지 후 재개). 이 오염이 판정 z값을 3.58(오염 제거)→1.80(오염 존재)까지 흔들었다. v5.242에서 `app.py`의 `_downcast()`에 방어를 넣었지만 `harness.py`의 자체 `_downcast()`(Close.notna만)는 이 방어를 받지 않아 프로덕션과 백테스트가 다른 데이터를 봤다. **2026-09-11 동기화 — 단, 단순 동기화(harness `_downcast()`를 app.py와 똑같이)는 틀린 방법이다:** `_filter_invalid_bars()`의 갈래B는 "가장 최근 5거래일+ 연속 무효구간 이전을 전부 버린다"라서, 오늘까지의 전체 이력에 먼저 적용하면 예컨대 2025년에 장기 정지된 종목이 2023년 체크포인트에서도 사라진다(미래 사건으로 과거 표본을 고르는 룩어헤드). 그래서 `harness.clean_at_checkpoint(trunc_df)`를 신설했다 — 내부에서 `from app import _downcast`(= `_filter_invalid_bars` + Close.notna + float32, 프로덕션 fetch 후처리 전체)로 **import 공유**(사본 금지)하고, 체크포인트까지 자른 df에만 적용해 그 시점 프로덕션이 봤을 데이터와 정확히 일치시킨다. fetch 시점 `_downcast()`는 기존 스크립트 재현성을 위해 그대로 뒀다 — **그래서 기존 스크립트는 여전히 무효봉을 포함한 데이터를 보며, 새 측정은 반드시 `clean_at_checkpoint()`를 거칠 것.** 근거와 설계: `docs/imminent_score_rank_vs_return.md` 1.7-6/1.8a. 배제 기준(OHLC 4필드, close_out_of_range·Volume=0은 제외 — 실측상 신뢰도 없음)과 갈래A/B(임계값 `_GAP_TRUNCATE_MIN_RUN=5`는 `scanner.select_pivot()`의 최단 lookback 5봉에 근거)는 app.py `_filter_invalid_bars()` docstring 참고. **같은 조사에서 발견한 harness↔프로덕션 차이 2건(미수정)**: ① 프로덕션의 KR 시총 1000억 미만 제외(`app._MCAP_MIN_EOK`, v4.91)가 harness에 없다 — 기존 측정 전부 이 필터 없이 쟀다(2026-09-11 측정은 현재시총×가격비 추정치로 자체 재현). ② `harness.fetch_kr_benchmarks()` 기본값(900일=601봉)으로 90개 체크포인트를 돌리면 `bench_score_at()`이 off 410~600은 0.0, off 610+는 자르지 않은 전체 시계열(룩어헤드)로 조용히 폴백한다 — 오염 스크립트 12개와 재검증 대기 목록은 `docs/kr_us_strategy_map.md` "사전 등록 대기 — 벤치마크 룩어헤드 재검증" 절.
- **`static/index.html`의 JS 판정 로직(단순 렌더가 아니라 조건 분기로 결과가 갈리는 계산)에 새 테스트가 필요하면 "텍스트 추출 + Node 실행" 패턴을 쓸 것 — 재구현(Python으로 같은 로직을 다시 짜서 비교) 금지, `node --check` 문법 검사만으로는 로직 자체를 검증 못 한다.** pytest는 JS를 직접 못 돌리다 보니 지금까지 프론트 로직 테스트는 `node --check`(문법만 확인, 값은 확인 안 함)에 머물러 있었다 — 리팩터링 중 판정 로직이 조용히 깨져도 이 레포 테스트 전체가 계속 초록불일 수 있었다는 뜻(발견 경위: v5.232에서 🔴 즉시행동 카드의 "(전일 종가 기준)"/"(전 거래일 종가 · 장전)" 표기 로직을 프론트→서버 계산으로 옮겼는데, 관련 실데이터(KR 즉시진입 카드)가 production에 하나도 없어 KR 분기가 한 번도 실행 검증된 적이 없었음 — `test_trace_parity.py`의 "셋업별 커버리지 0건이 통과로 표시" 구멍과 같은 종류, 사용자 지적으로 발견). **레시피(v5.233, `test_price_basis_note.py`가 실례)**: ① 판정 로직을 클로저에서 최상위 순수 함수로 승격(예: `priceBasisNoteText(item)`) — 입력을 인자로 받고 DOM/전역상태에 안 기대는 형태여야 격리 실행 가능. ② Python 테스트가 **정규식이 아니라 중괄호 깊이를 세어** 그 함수 텍스트를 `static/index.html`에서 그대로 추출(정규식만으론 중첩 중괄호가 있는 함수 본문을 안전하게 못 자름 — `test_trace_const_audit.py`가 AST를 쓰는 것과 같은 이유로 "텍스트가 우연히 맞아떨어지는" 상황을 피함). ③ `subprocess.run(["node", "-e", 추출한소스 + 테스트용 입력 리터럴 + "console.log(JSON.stringify(결과))"])`로 **실제 그 코드를 실행**해 stdout을 assert — 재구현이 아니라 production 코드 그 자체를 돌리므로 두 구현이 갈라질 위험이 원천적으로 없다. ④ 서버 쪽 계산(예: `is_live`/`market_open_now`를 만드는 `_price_basis_fields()`)도 같은 문제(실데이터가 한쪽 분기를 안 밟음)를 겪을 수 있다 — 이건 이미 Python이라 추출이 필요 없고, `monkeypatch`로 시간 의존 함수(`_is_market_open_now` 등)만 고정해 조합을 직접 만들어 검증(`test_price_basis_fields.py`가 실례, "프론트 테스트가 가정한 조합을 서버가 실제로 만들어낼 수 있는가"까지 짝으로 확인). ⑤ `node` 미설치 환경은 `pytest.skip`(hard FAIL 아님 — 도구 부재는 로직 결함과 다른 종류의 문제). **검증 관례**: 새로 만든 테스트가 진짜 탐지력이 있는지, 판정 결과를 일부러 바꿔치기해 테스트가 실제로 FAIL하는지 확인한 뒤 원복 — 통과만 확인하고 끝내면 tautology(항상 통과하는 테스트)를 못 걸러낸다.

- **사보타주가 통과하면 그건 코드가 안전한 게 아니라 테스트가 결함인 것이다. 사보타주 통과 시 테스트를 먼저 의심할 것.** 2026-09-14 한 세션에서만 3건 나왔고, 셋 다 "통과했으니 됐다"로 넘겼으면 탐지력 없는 테스트가 그대로 남았을 것이다. 반복되는 패턴 3가지:
  1. **`in` 존재 검사** — `assert "scoreRing(s.score)" in src`처럼 존재만 보면, **호출부가 2곳일 때 하나를 지워도 통과**한다(v5.261에서 실제 발생). → **개수로 고정**(`src.count(...) >= 2`). 같은 이유로 "이 문자열이 없어야 한다" 검사는 **주석·changelog의 인용문에 걸려 오탐**하기도 한다(v5.259에서 발생) → 검사 범위를 실행 코드 영역으로 좁힐 것.
  2. **순서 의존 검사** — "A를 바꾸면 B가 안 바뀌어야 한다"로 독립성을 보면, **A가 B보다 늦게 처리되는 경우 결합을 주입해도 안 잡힌다**(참조 대상이 아직 비어 있어 양쪽 다 같은 값이 됨, 측정 B 스크립트에서 발생). → **순서에 무관한 형태**로 바꿀 것: "B의 결과는 B의 입력만으로 결정되는가"(다른 입력을 전부 지워도 같은 결과인가). 더해서 **"그 항목이 조용히 전부 비는" 경우**를 잡는 검사를 따로 둘 것 — 비교만으로는 양쪽 다 비면 '같다'로 통과한다.
  3. **합성 데이터로 전부 채우기** — 테스트 픽스처가 모든 필드를 채워 넣으면 **실데이터에서 `None`으로 오는 경우를 재현하지 못한다**(측정 B에서 3개 항목이 전부 None이라 측정조차 안 됐는데 테스트는 초록불이었다). → **실데이터의 형태**(빠진 필드, 타입이 다른 필드 — 예: 불리언인 줄 알았는데 0~8 정수)를 픽스처에 포함할 것.
  4. **입력을 직접 만들어 넣은 재현** — 버그를 재현한다며 함수 인자를 손으로
     구성하면, **프로덕션이 그 인자를 만드는 경로에 있는 버그는 원리적으로 못 잡는다.**
     2026-09-17 이노메트리 백필 조사에서 `kr_data`를 매번 직접 만들어 넣어 **가설 셋을
     연속으로 헛짚었다**(resolved 차단 / `_meta` 필터 / `_downcast` 트렁케이션 — 전부
     로컬 재현 통과). 진짜 원인은 `_fetch_market_data_inner()`가 **fetch 이전에**
     시총 1000억 필터로 유니버스를 잘라 그 티커가 `bundle["data"]`에 아예 없던 것이었다
     (종가베팅 후보 기준은 거래대금 상위 100이라 시총 조건이 없다 — 정상적으로 섞인다).
     측정 B에서 `rs_3m`/`rs_delta`를 `analyze()`에 안 넘겨 3항목이 조용히 미측정된 것도
     **같은 유형**이다. → **재현은 입력 생성 경로부터 프로덕션과 같게 만들 것**
     (`test_jongga_bundle_fallback.py`의 `_production_bundle()`이 실례 — 시총 필터를
     프로덕션과 같은 순서로 적용한 뒤 검증한다).
  5. **침묵을 성공으로 읽지 말 것** — 로그를 `if changed:`로 감싸면 "아무것도 안 함"과
     "호출조차 안 됨"이 **똑같이 무음**이다. 2026-09-17 로그에 `[jongga-forward]` 줄이
     없는 것을 보고 "함수 미호출"로 오판했는데, 실제로는 돌았고 0건을 채운 것이었다.
     → 배치성 함수는 **0건일 때도 "대상 N건 중 0건 처리"를 남긴다**(v5.266).
  덧붙여, 측정 스크립트에서 **표본이 0이면 경고가 아니라 하드 실패**로 만들 것(위 "발견한 문제는 실패로" 원칙과 같음). B는 n=0 항목을 `nan`으로 조용히 넘겨 결과표에 0으로 남겼고, 그 사실을 사람이 표를 읽다 발견했다.
...
### 90cp 측정 실행 시각 규칙 (v2026-09-12, 사용자 지시)
**KR·US 각 시장의 마지막 봉이 확정된 뒤 실행할 것. 기준은 언제나 KST이고, 이 머신 시계(뉴질랜드)는 KST보다 앞선다 — **NZST(표준시) KST+3 / NZDT(서머타임) KST+4**. 2026-09-27부터 2027-04-04(4월 첫 일요일)까지는 NZDT다. 그래서 같은 파일 시각 10:31이 NZST 구간이면 KST 07:31, NZDT 구간이면 KST 06:31 — 어느 쪽이든 개장 전이다. 개장 전·장중 실행은 종목별 마지막 봉 유무가 갈려 재현이 어긋난다. 안전 시각: **KST 20:10 이후 ~ 22:30(미장 개장) 전** — 로컬로는 NZST 23:10~01:30 / NZDT 00:10~02:30.**

**⚠️ 로컬 시각으로 기억하지 말 것.** 서머타임 전환(9월 마지막 일요일·4월 첫 일요일)마다 로컬 창이 한 시간씩 밀린다. 판단은 KST로 하고 로컬은 환산값으로만 쓴다 — `harness.run_stamp()`이 `run_at_local`·`run_at_kst`·`local_tz_utc_offset_hours`를 다 남기므로, 과거 파일을 해석할 때는 **기록된 offset(+12=NZST, +13=NZDT)을 보고** 환산한다(그 시점에 서머타임이었는지 사람이 기억할 필요가 없다).

**⚠️ v5.263(2026-09-14)에서 창이 16:00→20:10으로 좁아졌다 — KRX 애프터마켓(16:00~20:00 KST) 도입 때문이다.** 실측(2026-09-14 19:34→19:35, 75초 간격 재조회)에서 naver siseJson **일봉의 Close·Volume이 애프터마켓 중 계속 갱신**됐다(삼성전자 거래량 +5,417주, SK하이닉스 종가 +1,000원). 즉 **15:30 종가는 더 이상 그날 봉의 끝이 아니다** — 16:00~20:00에 측정을 돌리면 마지막 봉이 미확정 상태로 들어간다. 20:10은 애프터 종료 + 여유 10분(임의값, 측정 근거 없음)이고 `app.KR_CLOSE_CONFIRMED_HM` 상수와 같은 값이다(EOD 스캔·jongga 기록·캘린더 KR↔US 전환이 전부 이 상수를 쓴다).

근거: 2026-09-07 측정 2건(8·9번 재검증 대상)이 KST 07:31/08:34 — KR 개장 전에 실행돼 KR 데이터가 09-04에서 끝나 있었다. 재검증 때 09-07 확정봉 기준으로 앵커를 잡자 R0 재현 게이트가 실패(박스돌파 KR −4.97%)했고, KR만 하루 앞으로 당기자 ±1.3% 안으로 재현됐다(`docs/display_only_bench_revalidation.md` §1-부록·§2). 실행 시각 전수 확인 결과 오염 스크립트 10개 중 6개가 개장 전·장중 실행이었다(표는 `docs/kr_us_strategy_map.md` "사전 등록 대기 — 벤치마크 룩어헤드 재검증" 절).
**새 측정은 `harness.run_stamp()`을 결과 JSON에 반드시 포함할 것** — 실행 시각(로컬+KST)과 KR/US 마지막 봉 날짜를 남겨, 다음 재검증이 파일만 보고 앵커를 잡을 수 있게 한다.
