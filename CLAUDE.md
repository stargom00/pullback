...
## 🛑 임시 제약 (2026-09-04 사용자 지시, 모든 세션 공통) — 해제될 때까지 이 섹션 최우선으로 읽을 것

**`app.py`/`scanner.py`에 "채택성" 변경(측정 결과를 근거로 프로덕션 동작을
실제로 바꾸는 커밋 — 예: `CONFIRM_RULE_BY_TAB` 확장, 게이트/스코어 로직
변경, 새 탭/필터 채택 등)을 커밋하지 말 것.**

해제 조건(둘 다 완료돼야 해제):
1. `scripts/measurements/2026-09-04_kr_confirm_entry_all_tabs_90cp_checks.py`
   측정 결과(슬리피지 +0.3%/+0.5% 재계산, 종목별 dedup n/z) 확인 완료 —
   `docs/kr_us_strategy_map.md` 최신 절에 반영 여부로 확인.
2. `docs/signal_snapshot_design_2026-09-04.md`(신호 스냅샷 설계) 기반 구현 완료.

**허용되는 것**: `scripts/measurements/` 아래 측정 스크립트 작성·실행·커밋,
`docs/` 조사·설계 문서 작성 — 막히는 건 `app.py`/`scanner.py`의 실제 동작
변경뿐이다. 위 두 조건이 끝나면 사용자가 이 섹션을 직접 지우거나 해제
지시를 함 — 세션이 스스로 판단해서 지우지 말 것.
