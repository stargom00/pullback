...
- **`/api/debug/{ticker}`의 modes/게이트추적 RS 판정: 콜드캐시일 때만 80 근사치 폴백, 캐시 웜이면 실제 rs_ranks 사용 (v5.61).** 유니버스 전체 캐시(`_fetch_market_data("all")`)가 따뜻하면 거기서 실제 rs_ranks/rs_moms를 가져오고, 콜드일 때만 80 고정 근사치로 폴백하며 이 경우 reasons에 `⚠️ RS 근사치(...)`로 명시됨. rs_min이 80~85 사이인 경계 종목(imminent/breakout/boxbreak는 rs_min=85)은 콜드캐시 상황에서만 이 근사치로 인해 디버그 패널과 실제 스캔 결과가 어긋날 수 있음.
