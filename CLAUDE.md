...
- **fetch 730일(KR)/2y(US) 확대(v5.28) 후 돌파/박스돌파/돌파임박 탭 회귀 미검증.** 눌림목·추세전환·ABC·`off_high_pct`·`rs_raw_score`·`count_bases_since_bottom` 등은 전체 유니버스 재현으로 회귀 없음을 확인했지만(400일→730일 hit 건수·순위 완전 동일), 이 3개 탭은 검증 당일 시세에 통과 종목이 0건이라 `max_off_high` 게이트가 실제로 걸러본 적이 없다. 해당 탭에 hit이 나오는 날 다시 확인 필요.
  - **ABC(문서 방식) 탭 설계(진행 중)에서 KR 유니버스를 추가로 1900일(≈1275봉)로 통일하기로 결정**(기존1500∪시총천억~10조=1927종목, 이원화 대비 fetch -33%·디스크 -7% — 실측 근거는 세션 기록 참고). 눌림목·추세전환 등은 730→1900일도 회귀 0건 확인(rs_raw_score/off_high_pct/count_bases_since_bottom corrections·is_first_base·bottom_ago 전수 일치, `low_lookback=250`이 1900봉 줘도 진짜 250봉 트레일링 윈도우로 동작함도 재확인). **단 돌파/박스돌파/돌파임박 3탭은 위 730일 확대 때도 미검증이었고 이번 1900일 확대도 마찬가지로 미검증** — 이 3개 탭에 hit이 나오는 날 두 확대(730일, 1900일)를 한 번에 확인할 것.
