...
- **데이터 소스(naver siseJson) 오염 방어가 `app.py`에만 있고 `scripts/measurements/harness.py`에는 없다 — 백테스트 재실행 전 반드시 동기화할 것.** 2026-09-10 종가베팅 측정(`scripts/measurements/2026-09-10_jongga_exit_timing.py`) 중 발견: naver siseJson API가 무거래일에 OHLC를 0으로, Close만 전일가로 이월해서 반환한다(예: `201490.KQ`, `007610.KS`는 2022-03~2026-04 약 4년치가 이 패턴 — 장기 거래정지 후 재개). 이 오염이 백테스트 변동성을 부풀려 판정 z값을 3.58(오염 제거 시)→1.80(오염 존재 시)까지 흔든 게 발견 경위. v5.242에서 `app.py`의 `_downcast()`(KR+US 공용, `_filter_invalid_bars()` 신설)에 OHLC 어느 필드든 0 이하/NaN/high<low인 행을 배제하는 방어를 추가했지만, **`harness.py`는 텍스트상 비슷한 자체 `_downcast()`를 따로 갖고 있어(app.py를 import하지 않음) 이 방어를 전혀 받지 않는다** — 이 방(세션)에서는 harness.py를 고칠 수 없다는 명시적 지시로 미수정 상태(사용자 지시). **결과: 지금 프로덕션 스캔과 백테스트가 서로 다른 데이터를 본다** — harness.py의 `_downcast()`를 app.py와 동기화(또는 import 공유)하기 전까지 이 상태가 유지된다. **다음에 종가베팅류 백테스트를 다시 돌리는 세션은 이 동기화부터 먼저 할 것.** 배제 기준(OHLC 4필드, close_out_of_range·Volume=0은 제외 — 실측상 신뢰도 없음)과 갈래A/B(산발적 행제거 vs 5거래일+ 연속시 트렁케이션, 임계값 `_GAP_TRUNCATE_MIN_RUN=5`는 `scanner.select_pivot()`의 최단 lookback 5봉에 근거) 설계는 app.py의 `_filter_invalid_bars()` docstring 참고.
