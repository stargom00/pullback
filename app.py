...
v5.288 [기능개선] `/api/dist/{ticker}` 응답에 **`bar_date`**(평가한 df의 마지막
    ...
    값은 시계가 아니라 **데이터**를 따르고, 날짜 인덱스가 아니면 None이다.
    판정은 v5.287 [1]에서 만든 `_last_bar_daykey()`를 그대로 재사용(사본 금지).
    이유: 봇이 "언제 자료로 판정한 값인지"를 응답만 보고 알 수 있어야 한다 —
    휴장일·거래정지·캐시 낡음이면 마지막 봉이 오늘이 아닐 수 있는데 예전
    응답엔 그 단서가 없었다(v5.287 [1]의 opening-surge 39배 사고와 같은 종류).
    [테스트] test_dist_bar_date.py(3) — 사보타주(키 제거) FAIL 확인.
...
v5.283 [UI·구조] 메인 메뉴 정리 + **시장 강제 탭 규칙 일원화**(사용자 지시).
    ...
    같은 규칙으로 접근된다. **위치만 이동** — 스캔·알림·forward 기록·내부
    키 전부 불변(탭 전환 핸들러에 이 세 모드 전용 분기가 원래 없었다).
    추세전환은 현 위치 유지, 기본 탭 변경 없음.
    [라벨] 눌림목 → **"US눌림목"**(표시만). `data-mode="pullback"`과
    `MODE_TAB_LABEL.pullback`(→ 일지 `tab:"눌림목"`)은 **그대로**다 —
    app.py의 EV 조회키(`("눌림목","KR")`)·분기(`tab == "눌림목"`)가 같은
    문자열을 쓰므로 키까지 바꾸면 기존 일지 레코드가 끊긴다.
    [시장 강제] `applyForcedMarket()` 신설 — **규칙을 한 곳에서** 처리.
    예전엔 종가베팅만 클릭 핸들러에 `setMarket('kr')`이 박혀 있었고
    **복원이 없어** 나온 뒤에도 kr이 남았다. 이제 두 탭 공통:
      진입 시 직전 market 저장 → 강제(jongga=kr, pullback=us)
      이탈 시 저장값 복원
      탭 안에서 사용자가 시장 버튼을 직접 누르면 복원하지 않음
      강제→강제 직행은 저장값을 **덮어쓰지 않음**(최초 비강제 탭 값 유지)
    ⚠️ 기존 `userMarketOverride`를 재사용하지 않았다 — 그건 **세션 전체**
    플래그라 "이번 방문 중에 눌렀는가"를 표현 못 하고, 과거에 한 번이라도
    누른 사용자는 영영 복원이 안 된다. 방문 단위 플래그를 따로 뒀다.
    [조사 결과 — 지시 일부 취소] **URL 해시 라우팅이 이 앱에 없다**
    (`location.hash` 0건, `hashchange` 0건). 딥링크 보존 항목·테스트는
    대상이 없어 삭제했다.
    [테스트] test_tab_market.py(14) — 메뉴 구조 6건 + 시장 규칙 8건
    (`applyForcedMarket`을 Node로 그대로 실행). 사보타주 5종(복원 제거 /
    직행 시 저장값 덮어쓰기 / 사용자 선택 무시 / 3개 중 1개를 최상위에 남김 /
    내부 키를 라벨과 같이 변경) 전부 FAIL 확인 후 원복.
    [묶어서 정정] ① v5.281 항목의 테스트 수 10→**17**, 사보타주 CH·CI·CK
    추가(CJ가 통과한 이유도 명시). ② test_disk_cache_protocol.py의 musl
    주석 — musl은 `CDLL` **OSError** 경로라 그 테스트(AttributeError)와
    다르다. **`trim=0`은 musl이 아니라 glibc에서 반환할 게 없었다는 뜻**이고,
    trim 기여는 플래그가 아니라 **B→C 차이(MB)**로 읽어야 한다.
...
v5.281 [성능] 평일 **장전(00:00~09:00 KST)을 "확정 거래일"로** 본다(사용자 지시).
    ...
    **+ 평일 휴장일도 종일 확정**(2026-09-23 추가). 1차는 장전만 덮어서
    ...
    [테스트] test_premarket_daykey.py(**17**) — 장전 10건(화 07:00→월 /
    월 07:00→금 / 화 10:00→None / 화 20:15→화 / 08:59↔09:00 경계 / 주말·US
    불변 / `_confirmed_daykey` 통과 / 상수 사용 / 기존 헬퍼 재사용) +
    휴장일 7건(휴장일 목록 전제 / 휴장일 장중→직전 거래일 / 휴장일 마감후 /
    연휴 뒤 월요일 / 거래일 장중은 None 유지 / `is_trading_day` 재사용 /
    US 불침범).
    사보타주: 장전 4종(장전 분기 제거=원래 동작 / 오늘 날짜 반환 / 직접
    -1일 계산으로 주말 못 건너뜀 / 경계를 20:10으로) + 휴장일 3종
    (**CH** 휴장일 반환 분기 제거 / **CI** 휴장일 판정 제거=원래 동작 /
    **CK** 평일 조건 제거로 주말까지 삼킴) 전부 FAIL 확인 후 원복.
    ⚠️ 휴장일 사보타주 1종(**CJ**: `_last_trading_daykey(now)` →
    `(now-1일)`)은 **통과했는데 탐지 실패가 아니다** — `kr_holiday`가 참이면
    오늘은 정의상 비거래일이라 시작점이 하루 달라도 답이 같다(두 날짜 실측
    확인). 사보타주 설계가 틀린 것이다.
...
v5.280 [UX·설계] 일지 3건(사용자 지시).
    [1] **종목코드 칸에 한글 종목명** → 코드 자동 해석. "대한항공" → 003490.KS.
        ...
        그대로 재사용한다 — 프론트에 두 번째 매칭 규칙을 만들면 서버와 갈라진다
        (테스트가 `includes/startsWith/indexOf` 직접 매칭을 금지한다).
        시장은 **코드에서** 판정(`.KS/.KQ` → KR)하므로 **종목명만 보고 US로
        오판하던 문제도 같이 해결**된다. 후보가 여러 개면 고르게 하고
        멋대로 정하지 않는다.
    [2] **목록을 통계 카드 위로.** "내가 지금 뭘 들고 있나가 스크롤 없이 보여야
        한다" — 통계 4칸 + 캘린더 + R곡선 + 신호검증 + 페이퍼추적이 목록보다
        위에 있어 보유를 보려면 매번 스크롤해야 했다. 기본 탭은 원래부터
        'entered'라 이제 열면 보유가 첫 줄이다.
    [3] **시장 게이트가 기록을 막지 않는다.** 게이트 🔴이면 "+일지" 진입 저장이
        "관찰(대기)로만 가능"으로 강제 전환됐고, **이미 산 종목의 진입 기록이
        불가**해 실전 R이 유실됐다 — 기록과 억제를 섞은 것이다(CLAUDE.md
        "기록과 억제는 분리"). 이제 경고는 그대로 두되 **진입으로 저장**하고,
        역행 사실을 `gate_defiance`에 남긴다(나중에 게이트 순응/역행 EV 비교의
        데이터). `markEntered()`(대기→진입)도 같은 차단이 있어 같이 풀었다.
        대시보드 표시용 `entryGate()` 호출은 차단이 아니라 그대로 둔다.
    [테스트] test_journal_ux.py(13). 사보타주 5종(강제 대기 전환 복구 / 역행
    태그 제거 / 한글 해석 제거 / 시장 US 하드코딩 / 통계를 다시 위로) 전부
    FAIL 확인 후 원복.
    [정리] 게이트가 기록을 막지 않게 되면서 `_ENTRY_GATE_BYPASS_HINT`
    ("이미 매수했다면 '+직접 추가'에서 진입으로 기록하세요")가 **죽은 상수**가
    됐다 — 우회할 대상이 없다. 상수·낡은 주석 모두 삭제하고, 그걸 검사하던
    테스트 3건을 새 설계(경고는 하되 저장은 허용)로 다시 썼다.
    ⚠️ 그 전환에서 `test_mark_entered_alert_includes_bypass_hint`가 수정 후에도
    **통과**했는데, 실행 코드가 아니라 **낡은 주석**에 남은 상수 이름에 걸린
    tautology였다 — 문자열 존재 검사로 동작을 확인하면 이렇게 조용히 무력해진다.
    [이번에 반복된 오탐 3종 — 전부 "검사 범위" 문제]
      ① 사보타주가 "통과" → 실은 **패치가 안 먹음**(들여쓰기 4칸 vs 2칸).
         사보타주 통과 시 테스트를 의심하기 전에 **앵커가 맞았는지** 먼저 볼 것.
      ② 내가 쓴 **변경 이력 주석**("예전엔 …진입 대신…")에 걸려 고친 코드가
         안 고쳐진 것처럼 보임 → 검사를 실행 코드로 좁힘(`_code_only`).
      ③ 함수 끝을 `text.index("async function", …)`로 잡아 **다음 함수까지
         삼킴** → 중괄호 깊이 추출로 교체(test_price_basis_note.py 방식).

v5.279 [버그수정] 일지 배지 2종이 **계산만 되고 화면에 안 그려지고 있었다**
    (사용자 발견 — "표시는 뜨나? 안 뜨면 그게 버그").
    · v5.274의 **"피벗 도달 ↑"** — 자동 진입을 없앤 자리에 "도달 사실은 표시로
      남긴다"가 설계였는데, `const reachedBadge = …`를 만들어 놓고 **출력 어디에도
      안 넣었다**. 그래서 pending이 피벗에 닿아도 **아무 변화가 없었다**.
    · v5.277의 **"상태 미기록"** — 같은 실수(정의 1회, 사용 0회). status가 빈
      레코드 2건이 화면에서 계속 안 보였다.
    둘 다 상태 셀(`viewRow`)에 배선했다.
    [구조 방어] 같은 실수가 **두 번** 났으므로 문자열 검사로 끝내지 않는다 —
    `viewRow` 안의 `const …Badge =`를 전부 뽑아 **`${…}`로 출력에 들어갔는지**
    대조한다(test_every_badge_const_in_viewrow_reaches_the_output). 앞으로
    새 배지를 만들고 안 그리면 그 자리에서 FAIL한다.
    [전제 정정] 사용자가 "프로덕션은 v5.270이라 v5.274 미배포"로 보고했으나
    **프로덕션은 v5.277이고 v5.274가 포함돼 있다**(배포본 확인: 자동진입 대입
    없음, `pivot_reached` 존재). 즉 자동 진입 제거는 이미 살아 있었고, 안 보이던
    건 **표시 배선**뿐이었다.
    [테스트] test_journal_no_auto_entry.py 9→12. 사보타주 3종(두 배지 배선
    제거=원래 버그 재현 / 하나만 누락 / 새 배지 정의만 추가) 전부 FAIL 확인 후 원복.
...
v5.276 [신규] 🩷 MA600 첫 돌파 **사건** 표시(사용자 지시).
    ...
    · **C 단계와 독립**이다 — C는 "지금 어디 있나"(상태), 이건 "언제 넘었나"
      (사건)라 같은 종목이 C3여도 최근 10봉 내 돌파면 잡힌다. C 분기 **밖**에서
      계산하며, 안으로 옮기면 test_gate_break_is_independent_of_the_c_stage가 FAIL.
    · `_find_gate_break()` 신설 — MA200을 보는 `_find_breakout()`과 **다른 선**
      이다. 둘을 한 함수로 합치면 어느 선 기준인지가 호출부에서 사라져 분리했고,
      서로의 기간 키를 안 쓰는지 테스트로 고정했다.
    · 최근 10봉 내 **첫** 돌파만. 거래량은 돌파봉 포함 3봉 중 최대 ÷ 50일 평균
      (교차봉이 소량이고 다음날 대량이 터지는 형태가 흔하다 — v5.267과 같은 이유).
    · `gate_break_window`(10) · `gate_break_vol_mult`(1.5) · `gate_break_vol_bars`(3)
      · `gate_break_vol_avg`(50) 전부 **임의값**, ABC_CONFIG 한 곳.
    [사보타주] 5종 중 **1종이 또 통과**했다 — "첫 돌파에서 멈추는가"를
    `assert "break" in ...`로 검사했는데 함수 안의 `cfg["gate_break_vol_avg"]`에
    "break"가 들어 있어 **항상 참**이었다. v5.272 `_find_breakout`에서 똑같은
    실수를 했고 **두 번째**다 — 창 안에 돌파 2회를 만들어 앞의 것을 잡는지
    **동작으로** 검증하도록 바꾼 뒤 재시도해 탐지 확인.
    ⚠️ 교훈: **텍스트로 제어흐름을 검사하지 말 것.** 함수명·설정키에 그 단어가
    들어 있으면 조용히 tautology가 된다.
...
v5.271 [조건추가] ABC 등급에 역배열·거래대금 상한 2건(사용자 지시, 둘 다 임의값).
    · **역배열**: MA200 < MA600이면 C2에서 진돌이/가돌이를 안 매긴다
      ...
      None으로 만들면 종목이 탭에서 사라진다. 가격 위치가 아니라 두 이평의
      **순서**를 보는 축이라 C 단계와 별개다.
    · **거래대금 상한** `max_turnover_eok=1000`억 초과 → "대형", A급을 B급으로
      내리는 **천장**(미달이 아니다 — fails에 넣으면 C급까지 떨어진다).
      최초 지시는 상한도 300억이었는데 하한과 같아 **A급 구간이 300억 한 점**으로
    ...
    [실측 — 두 조건이 실제로 하는 일] KR 403종목 표본, ABC 후보 153건:
      ...
      대형 8건(5%)은 **전부 이미 C3 이탈**이라 **등급 변경 0건** — 천장이
      한 번도 작동하지 않았다.
      거래대금 미달(<300억)이 126건(82%)이라 A급 가능 구간은 19건뿐이고,
      그 구간의 A급은 **POSCO홀딩스**였다 — "소형 성장주"라는 전제와 반대다.
      → 상한이 아니라 **하한 300억이 대형주를 고르고 있다.** 재검토 필요.
    [테스트] 역배열 4종(탐지·진돌이 차단·단계 유지·거래량으로 못 뚫음),
    거래대금 5종(상한>하한, 밴드, 상한초과는 미달 아님, B 천장, C급을 올리지
    않음). 사보타주 5종(역배열 차단 무력화 / 천장 제거 / 상한=하한 /
    상한초과를 미달로 / 부등호 반전) 전부 FAIL 확인 후 원복.
