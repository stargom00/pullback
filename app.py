...
v5.264 [의미 확정+UI] 종가베팅 "종가" 정의 확정, **매수 시각을 애프터마켓으로**.
        ...
        **248,500** 대조, 사용자 확인) → **백테스트 276건과 포워드 close_price의
        정의가 일치**한다. v5.263에서 "미확정"으로 달아둔 콜아웃을 확정으로 교체.
        실측 뒷받침: 애프터 진행 중(19:34→19:35) 일봉 Close·Volume이 갱신됐고,
        종료 후(22:08→22:10) 3종목 모두 변화 0으로 고정됐다 — **20:00에 멈춘다.**
        [핵심] **15:20 동시호가 가격은 정규장 마지막 가격일 뿐 "종가"가 아니다.**
        그 시각에 사면 백테스트가 가정한 가격(20:00 확정 종가)과 다른 값에
        진입한다 → 실전 진입을 **19:50~20:00 애프터마켓**으로 옮긴다.
        **후보 선정 시점(14:40~15:00 스냅샷)은 그대로** — 매수 시각만 바뀐다.
        [화면] `_jongga_session_state()` 활성 창 **15:30 → 20:00**, 문구
        "⏰ 오늘 15:20 동시호가 전 진입용" → **"⏰ 오늘 19:50~20:00 애프터마켓
        진입용"**. `TODAY_DECISION_INFO`·GUIDE.md 6.5장도 같은 취지로 갱신.
        [JONGGA_READY_HM 재검토 — **15:20 유지**(사용자 요청에 대한 판단)]
        진입이 19:50로 갔지만 이 문턱의 의미는 진입 시각이 아니라 **"후보가
        나왔는가"**다(즉시행동 0건 사유 안내용). 19:50으로 올리면 15:00~19:50
        사이에 **후보가 화면에 떠 있는데도 "아직 없다"는 안내가 뜨는 모순**이
        생긴다. 값은 두고 문구만 "종가베팅 **후보는** 15:20 이후"로 고쳐
        진입 시각으로 오해되지 않게 했다.
        [관찰 과제] 애프터마켓은 호가가 얇아 15:20 동시호가와 체결 여건이 다르다 —
        백테스트는 종가에 무조건 체결된다고 가정하지만 실전은 아닐 수 있다.
        **포워드 30건에서 체결가와 종가의 괴리를 관찰**할 것(문서에 명시).
        [테스트] test_jongga_ready_time.py +7(총 13) — 활성 창이 20:00까지인지,
        라벨이 애프터마켓을 가리키고 "동시호가"가 없는지, 후보 선정 시각(14:40)은
        그대로인지, 문턱이 15:20에 남아 있는지, 사유 문구가 "후보는"인지,
        문서 2종이 확정 문구인지. v5.260 테스트의 앵커가 문구 변경으로 깨진 것도
        같이 고쳤다. 사보타주 3종(활성 창 복귀 / 문턱 19:50 / GUIDE 복귀) 확인.
...
v5.256[버그수정] 섹터 칩이 시장 필터를 안 따라감(사용자 보고 — 눌림목에서
        ...
        시장 필터는 static/index.html의 renderCards()에서 클라이언트로만
        걸린다 → 둘이 만나는 지점이 없었다. **눌림목 전용이 아니라 칩을 쓰는
        전 탭 공통 버그**라 공통 지점(renderCards) 한 곳에서 고쳤다.
        [수정 — static/index.html만, 이 파일은 VERSION/이력만]
        · sectorSummaryOf(list) 신설 — 화면에 실제로 보이는 목록으로 재집계.
          정렬·하한(n>=2)은 서버 집계와 같은 규칙(첫 로드 시 칩이 달라 보이지
          않게).
        · 호출 위치: 시장·검색·주도주·💎·👑 등 **다른 필터를 전부 적용한 뒤**,
          **섹터 필터를 적용하기 전**. 섹터 필터까지 반영해 집계하면 칩을
          하나 누르는 순간 그 칩만 남아 다른 섹터로 못 옮긴다
          (renderPatternSubTabs와 같은 순서 원칙).
        · 칩 클릭/펼치기는 상태만 바꾸고 renderCards()에 위임 — 옛 summary로
          직접 다시 그리면 시장 필터 반영 전 목록이 한 프레임 남는다.
        · 로드 시 renderSectors(data.sector_summary) 호출 제거(중복 + 전체
          시장 칩 번쩍임). 서버 응답의 sector_summary 필드 자체는 유지.
        · [같이 막은 갇힘] 시장을 바꿔 선택된 섹터가 그쪽에 아예 없으면
          "0종목 + 해제할 칩도 안 보임"이 된다 → 히트가 남아 있으면 하한
          미만이어도 칩을 끼워 해제 가능하게, 한 건도 없으면 자동 해제.
        [테스트] test_sector_chip_market_filter.py(10) — 시장별 집계 분리,
        하한·정렬 규칙, 섹터 미분류 제외, 집계가 시장 필터 뒤·섹터 필터 앞인지,
        칩 클릭 위임, 갇힘 방지. 사보타주 3종(집계를 시장 필터 앞으로 / 시작
        목록에 섹터 필터 / 하한 n>=1)으로 검증 — **2번째는 처음에 통과해버려
        테스트에 탐지력이 없었고**(순서만 봐서 "앞에 하나 더 거는" 변형을 못
        잡음), 목록을 실제로 교체하는 지점 수까지 세도록 보강한 뒤 잡혔다.
        전체 649 passed.
...
v5.253[UI 개선] ⛔ 피벗 밀착(0~0.5ATR) 종목을 돌파·박스돌파 탭에서 기본
        ...
        −3.80 · 박스돌파 US −2.66/−2.66 · 돌파 KR −6.11/−5.24 · 돌파 US
        −3.82/−3.82), EV −0.21~−0.36R.
        [범위] 그 4셀 = 돌파·박스돌파뿐. 눌림목은 8번 측정 대상이 아니었고
        9번에선 저ATR 표본 부족으로 "판정불가"로 명시돼 있어 확장하지 않는다
        (최초 지시는 눌림목 탭이었으나 근거 범위를 넘는다는 확인 후 2026-09-13
        사용자가 "돌파·박스돌파 탭만. 근거 범위 그대로"로 확정).
        [동작] 필터 토글이 아니라 기본 접힘 — 탭 하단에 "⛔ 피벗 밀착
        (0~0.5ATR) N건 — 펼치기 ▼" 줄이 항상 보이고 누르면 카드가 펼쳐진다
        (pivotHugExpandedModes, 탭별 상태·미저장이라 새로고침하면 다시 접힘).
        기존 가격고정(price_frozen) 숨김 줄과 같은 패턴·독립 상태.
        모든 필터(시장·검색·주도주) 적용 뒤에 가르므로 N은 현재 화면 조건
        기준 건수. 히트가 전부 밀착이면 "조건 충족 종목 없음" 대신 전용 안내.
        게이트·스캔·저널·API 무영향(카드 렌더링 전용). ⛔ 배지 자체(v5.210)와
        신호등 pivot_hug 체크는 무변경.
        [테스트] tests/test_pivot_hug_collapse.py — 프로덕션 함수
        pivotHuggingDistAtr을 중괄호 깊이로 추출해 node로 실행(재구현 금지,
        CLAUDE.md 레시피). 경계(0.5ATR 배타, ATR은 close 기준)·적용 범위·
        기본 접힘 방향을 검증. 사보타주 2종(눌림목 확장, 조건 반전) 실패
        확인 후 원복.
...
v5.244 [수정] R 게이트 경고줄 오표시 + 차단 다이얼로그 우회로 안내
        ...
        자체는 설계 의도대로 정상 작동(v4.47.0 "감정매매를 구조로
        차단") — 건드리지 않음. 대신 두 가지 별개 문제를 확인:
        ① `renderCalcLine()`이 "+일지"/"+직접 추가" 공통인데, 실제로는
        `saveManualAdd()`("+직접 추가")가 `entryGate()`를 아예 호출하지
        않아(v4.52.3에서 "이미 매수한 종목 기록 못 함" 문제를 풀려고
        의도적으로 게이트를 안 붙인 설계 — 이번에도 안 건드림) 저장이
        실제로 막히지 않는데도, 이 모달의 실시간 경고줄이 "+일지"와
        똑같이 "→ 저장은 관찰(대기)로만 가능"을 보여줘서 화면이 사실과
        다른 말을 하고 있었다(사용자가 이걸 보고 실거래 기록을 시도조차
        안 하게 됨). ② `saveJournal()`/`markEntered()` 차단 다이얼로그에
        "이미 매수했다면 어떻게 기록하면 되는지" 안내가 없었다.
        [수정] ① `effectiveCapR(ignoreGate=false)`/`entryGate(newR,
        {ignoreMarketGate=false})`에 옵션 신설 — true면 시장게이트
        (조정/압박) 사유와 그로 인한 cap 축소(조정=0/압박=1.5R)를
        건너뛰고, 월간/주간 서킷브레이커·오픈리스크 상한(사용자 자신의
        포트폴리오 객관적 상태, 시장 해석과 무관하다고 판단 — 설계
        보고 참고)은 그대로 평가한다. `renderCalcLine(elId, entry,
        stop, market, gateOpts)`에 5번째 인자 추가, 호출부(`hookCalc
        Inputs()`)가 "+직접 추가"(`maCalc`)에만 `{ignoreMarketGate:
        true}`를 명시적으로 전달 — elId 문자열 매칭으로 암묵 분기하지
        않고 호출부가 자기 모달의 성격을 그 자리에서 드러내게 함.
        시장게이트 사유가 없을 때만 "저장은 관찰(대기)로만 가능"
        강제 문구도 같이 뺀다(이 모달에서는 사실이 아니므로) — 단
        오픈리스크/서킷브레이커 사유 자체는 "+직접 추가"에서도 참고
        정보로 계속 표시(3번 판단, 빼면 진짜 리스크 경고까지 같이
        사라짐).
        ② `saveJournal()` 차단 confirm과 `markEntered()` 차단 alert에
        공용 상수 `_ENTRY_GATE_BYPASS_HINT`("이미 매수했다면 '+직접
        추가'에서 '진입'으로 기록하세요") 추가.
        [판단 — 빼기 vs "미적용" 표시] "+직접 추가"에서 게이트 관련
        문구를 아예 안 보여주는 쪽으로 결정(사용자 의견과 일치).
        "미적용"이라고 명시하면 그 자체로 "게이트 걸린 종목도 여기로
        오면 된다"는 우회 안내처럼 읽힐 위험이 있고, 애초에 적용 안
        되는 규칙을 보여줄 이유가 없다.
        [알려진 부수효과 — UI 문구엔 반영 안 함, 여기 기록] `markEntered()`
        가 막혀서 "+직접 추가"로 새 진입 레코드를 만들면, 기존 대기
        (pending) 레코드는 그대로 남고 진입(entered) 레코드가 별개로
        하나 더 생겨 같은 티커에 두 건이 남을 수 있다. 이 앱은 "같은
        종목 복수 기록"을 이미 정상 패턴으로 지원하므로(saveJournal()
        상단 "별개의 새 기록으로 추가할까요?" 확인창이 선례) 문제는
        아니지만, 안내 문구엔 이 설명을 안 넣기로 함(간결하게 유지) —
        나중에 "왜 중복 레코드가 생기지"를 다시 조사하는 일이 없도록
        여기 남겨둠(사용자 지시).
        [검증] `entryGate()`/`effectiveCapR()`를 실제 소스 그대로 추출해
        Node로 실행 — correction/pressure 양쪽에서 ignoreMarketGate가
        시장게이트만 우회하고 진짜 오픈리스크 초과·주간/월간 서킷브레이커는
        여전히 막는지 확인. 특히 "open=1R+신규1R=2R, 기본cap 3R은 안
        넘지만 correction cap(0R)/pressure cap(1.5R)은 넘는" 조합으로
        "시장게이트가 오픈 리스크 상한 초과로 변장해 재차단"하는 함정을
        직접 재현·검증(사용자가 지목한 핵심 케이스) — `effectiveCapR()`
        호출에서 `ignoreMarketGate` 전달을 끊는 사보타지로 이 함정이
        실제로 재현되고 테스트가 정확히 FAIL하는 것을 확인 후 원복.
        `renderCalcLine()`도 가짜 DOM 스텁으로 실제 소스를 그대로 실행해
        "+일지"/"+직접 추가" 양쪽의 실제 렌더 결과(HTML 문자열)를 검증.
        안내 문구 사보타지(saveJournal()에서 힌트 제거) 1회 추가 검증.
        test_entry_gate_bypass_note.py 17건 신설. 전체 테스트 472건
        통과(기존 455 + 신규 17).
v5.243 [방어] "+직접 추가" 모달(saveManualAdd) 저장 시점 티커 형식
        검증 — 심볼로 불가능한 문자가 journal에 그대로 저장되던 경로
        차단 (사용자 지시, 2026-09-10 "한미사이언스" 오염 사고 후속).
        [확정된 원인] `/api/prices` 반복 호출 로그(`$한미사이언스: No
        data found`)를 역추적해 railway ssh로 프로덕션 journal_user.json
        을 직접 확인 — 151건 중 1건이 `ticker:"한미사이언스"`(심볼로
        불가능한 한글), `name:"008930.KS"`(실제 티커)로 저장돼 있었다.
        `saveManualAdd()`("+직접 추가" 모달)는 "티커"/"종목명" 칸이
        완전히 독립된 자유 입력이고 저장 시점에 티커 형식을 전혀
        검증하지 않아, 두 칸 값이 사용자 실수로 뒤바뀐 채(또는 그
        외 임의 문자열이) 그대로 journal에 들어갔다. 이 레코드는
        `status:'entered'`+`market:'US'`(`_maInferMarket()`이 KR
        패턴에 안 맞는 문자열을 기본값 'US'로 분류)라 `_isPriceTrackable
        ()`에 항상 걸려, 페이지 로드·60초 폴링마다 `/api/prices` →
        (`naver_kr.is_kr()`도 접미사 없어 False) → yfinance 경로로
        흘러들어가 매번 같은 실패를 반복 재현했다. `myTrackSaveAdd()`
        ("내 트래킹 ➕", 이름 검색 자동완성 있음)는 이번 오염과 무관함을
        직접 확인(pivot_type 등 필드 값이 saveManualAdd() 경로와만
        일치) — 그쪽은 이번 수정 범위에서 제외(사용자 지시, 확인 안 된
        곳까지 같이 고치면 범위만 커짐).
        [수정] `static/index.html`에 순수 함수 2개 신설 — `_isValidTicker
        Format(ticker)`(형식 검증만, KR 5~6자리 숫자코드/KR코드+.KS·.KQ/
        영문시작 영숫자+.-^ 1~10자만 허용, 그 외(한글·공백·기타 비ASCII)
        전부 거부) + `_maTickerFormatCheck(ticker, nameVal)`(검증 실패
        시 알림 메시지 조립, `nameVal`이 티커 형식으로 보이면 "칸이
        바뀐 것 같다"는 안내를 조건부로 덧붙임 — 별도 감지 상태 없이
        메시지 한 줄로만 처리, 저장 차단 여부는 `ticker` 자체의 형식
        하나로만 결정해 스왑 힌트가 오탐이어도 저장에 영향 없음).
        `resolve_name_to_ticker()`(이름→티커 해석)는 붙이지 않음 —
        `saveManualAdd()`는 원래 티커/종목명이 독립 자유입력인 모달이라
        (b)를 붙이면 모달 성격 자체가 바뀌는데, (a) 형식 검증만으로
        이번 사고 클래스는 전부 막힌다고 판단(설계 보고, 아래 [검증]
        의 유니버스 전수 확인 근거).
        [판단 — 두 번째 방어선] `_maInferMarket()`의 기본값 'US'는
        그대로 둠 — 형식 검증이 저장 게이트에 들어가면 이 함수는
        이미 심볼 형태로 좁혀진 문자열만 받으므로(한글 등은 그 전에
        거부됨), 'US' 기본값이 무력화될 조건 자체가 사라진다. 코드
        변경 없이 "이미 막힌 케이스에 대한 중복 방어"로 판단.
        [3단계 — 기존 오염 레코드] 삭제 후 재입력으로 처리(사용자 직접
        수행, 승인됨). 편집 UI는 추가하지 않음 — 1건 때문에 새 화면을
        만들 이유가 없고, 편집 기능이 생기면 잘못 고칠 여지도 같이
        생긴다는 사용자 판단. 현재 UI에 ticker/name/market을 고치는
        화면 자체가 없음을 확인(editRow()/saveEdit()의 인라인 편집
        폼은 entry/stop/exit/카테고리/수량/보유일/메모만 노출, ticker/
        name/market은 읽기전용 표시뿐)했고, 날짜(date) 필드도 편집
        불가라 재입력 시 원래 진입일(2026-09-05) 유실은 감수하기로
        함(사용자 지시).
        [검증] `_isValidTickerFormat()`을 KR 유니버스(1505) + US
        유니버스(2120) 전체(공개 시장 데이터, get_universe() — 개인
        거래 기록인 journal과 달리 커밋 대상)에 통과시켜 거짓 배제
        0건 확인(BRK-B/단일문자 V·F 등 유니버스 실존 형태 포함). 별도로
        pull해둔 프로덕션 journal_user.json 실제 레코드 150건(정상)
        전수도 세션 내에서 같은 방식으로 확인(거짓 배제 0건) — 이
        파일 자체는 개인 거래 기록이라 테스트에 커밋하지 않음, 영구
        회귀 테스트는 위 공개 유니버스 전수로 대체(journal 종목들의
        상위집합). test_ma_ticker_format.py 19건 신설(문자열추출+Node
        실행 레시피, CLAUDE.md 참고). 사보타지-복구 2회(형식검증
        항상 통과로 무력화, 스왑힌트 조건 강제 비활성화) 모두 해당
        테스트가 정확히 FAIL하는 것을 확인 후 원복. 전체 테스트
        455건 통과(기존 436 + 신규 19).
...
v5.230 [버그수정] 📌 내 추적 보드 종목명에 트레이딩뷰 링크 추가(사용자
        ...
        링크가 아예 없었음(활성/트리거 행만 펼침 패널 안에 "📈 차트"
        버튼이 있었으나 행을 펼쳐야만 보임, 재점화·트리거미입력 행은
        그마저도 없었음). 기존 카드와 같은 규칙(`tvUrl(ticker, market)`
        — KR은 KRX:코드, US는 심볼 그대로)으로 `class="name-link"` 링크를
        세 곳(재점화/활성·트리거/트리거미입력) 전부에 추가. 활성 행은
        바깥 div에 `onclick="toggleMyTrackExpand(...)"`가 있어 링크에
        `event.stopPropagation()`을 같이 걸어 클릭 시 펼침 토글이 같이
        발동하지 않게 함(재점화/트리거미입력 행은 바깥 클릭핸들러가
        없어 불필요).
...
v5.225 [기능추가] 캘린더 탭 종목검색 — 5탭 미해당 사유 표시(사용자 지시,
        ...
        차트링크만"으로 뭉뚱그렸던 것을, 5탭 각각의 탈락 핵심사유
        한 줄씩으로 교체 — `/api/debug`가 이미 반환하는 탈락_핵심사유를
        그대로 파싱(기존 _calShortReason 재사용, 새 계산 없음).
        [2] 기본 접힘("▾ 탈락 사유 (0/5)"), 클릭 시 펼침 — 5줄이 항상
        펼쳐져 있으면 압축뷰 취지(v5.224)와 어긋나 토글로.
        [3] 헤더 줄에 공통 지표(RS/ATR/200일선 이격) 추가 — 셋 다 이미
        응답에 있던 필드 재사용(신규 계산 없음): `rs_percentile`,
        `atr_median_pct`(이미 payload 최상위에 존재했음 — 새로 안
        만듦), `indicators.close_vs_ma200_pct`. 예:
        "008930.KS KR · 57,000원 · 제약 · RS 87 · ATR 6.1% · 200MA +43%".
        값이 null인 필드(신규상장 등 ma200 미달)는 자동으로 생략.
        검증: node --check로 인라인 스크립트 전체 문법 확인.
...
v5.217 [UI 전면개편] 홈 탭(캘린더) 재배치 + 📌 정리(사용자 지시,
        ...
        관찰) 숨김" 조건은 조사 결과 폐기(journal_user.json에 pending/
        watch가 생기는 경로는 전부 클릭이 필요, 서버 단독 자동등록
        경로 없음 — auto_watch/재점화는 별도 파일이라 이 보드 대상도
        아님). 대신 my_trigger_price 유무로 나눔: 있으면 활성 보드(최대
        5행, 상태: 📍도달/⚠️데이터확인[트리거-현재가 괴리 50%초과, 판정
        제외]/👀접근중/⌛만료임박/💤대기), 없으면 "▾ 트리거 미입력 N건"
        접힘(상태판정 없음 — 피벗 폴백 폐기, 펼치면 행마다 트리거
        입력칸+개별 "관찰 종료"+상단 일괄 종료). leader_conversion(👁)은
        트리거가 있을 수 없어 자연히 미입력 그룹에 들어감. 같은 티커로
        여러 건 쌓이면 최신 1건만 남기고 나머지 archived(중복 사유
        명시) — onEnterCalendarTab에서 _archiveExpiredPending과 같은
        타이밍에 _archiveDuplicateTrackers()로 처리.
        [B] 홈 탭 순서 재배치: 상단 고정 스트립(position:sticky, 게이트
        KR/US/종합 요약+포지션 건수·R합+📌도달 건수+🔴 건수, 클릭하면
        게이트 상세 펼침) → 1.📌내추적 2.🔴즉시행동(최대5) 3.🔎후보(관심·
        근접·이미돌파·눌림목후보·대기 통합, 기본 접힘·건수만) 4.📊섹터
        흐름 — 여기까지 "첫 화면" 목표. 이어서 5.📝메모(오늘 메모 1줄
        미리보기, 클릭해야 편집 textarea 펼침 — 포커스 유지를 위해
        여전히 별도 DOM) 6.📅D+14일정(기본 접힘, 오늘·내일 High중요도만
        접힌 채로도 미리보기). "🎯 오늘의 결정" 단일 컨테이너는
        renderTodayDecisionHtml()이 {immediateHtml, candidateHtml,
        immediateCount, candidateCount} 객체를 반환하도록 바뀌면서
        🔴/🔎 두 독립 섹션으로 분리(문자열 하나 반환하던 기존 방식에서
        변경, 호출부 renderCalendar만 영향).
        삭제: 오늘의 액션 큐(📌·🔴와 중복), 강세테마×스캐너(섹터흐름과
        중복). 이동: 시장 게이트 상세 박스(→상단 스트립+클릭 상세),
        포지션 카드(→상단 스트립 요약+포지션 탭, 홈 제거), 미국 요약
        문장(돈의흐름 final_sentence, →게이트 스트립 hover 툴팁).
        검증: node --check로 전체 인라인 스크립트 문법 확인, 상태
        판정(도달/접근중/데이터확인/만료임박)·중복 아카이브 로직을
        Node로 격리 테스트(9+2 케이스 전부 통과), `python3 -c "import
        app"`로 서버 임포트 확인. 1440px 화면에서의 실제 스크롤 높이·
        실 데이터 중복(예: 한화생명) 정리는 로컬에 시장데이터·로그인이
        필요해(CLAUDE.md 검증 방법 참고, 라이브 curl 불가) 배포 후
        사용자 확인 필요.
...
v5.213 [버그/근거 재검토] entrySignal 🟡주의 배지 제거 — US 눌림목
        ...
        무변경(이 배지 자체가 게이트가 아니었음).
        [조사] 🟡주의는 시장 지수 게이트(🟢🟡🔴)와 무관 —
        `entrySignal()`이 종목 단위로 계산하는 개별 신호등이었다.
        눌림목 탭은 판정 항목이 RS 하나뿐(`ENTRY_SIGNAL_ITEMS.pullback
        =['rs']`)이고 US 눌림목 게이트가 이미 RS≥80을 요구해, 남는 건
        RS 90+(🟢)냐 80~89(🟡)냐뿐 — 2026-09-08 실측(US 눌림목 RS≥80
        게이트 통과 103건)에서 정확히 50:53(🟢:🟡)으로 거의 50:50이라
        "12건 전부 🟡"는 이 모집단의 자연스러운 분포에서 그날 쏠린
        결과로 보인다(버그 아님). 하지만 RS90+/80~89의 EV를 직접
        비교한 측정이 문서에 전혀 없고, 이 RS-only 판정 설계의 근거였던
        "Script A" 원본 데이터 자체가 v5.67 노트에서 "재현 안 됨"으로
        격하된 상태 — 반면 같은 탭(US 눌림목)에서 캐비어트 신호가
        거꾸로였던 확정 사례(지수 게이트 반전, z=-3.15)가 이미 있어
        검증 안 된 이 배지를 계속 노출하는 리스크가 크다고 판단.
        docs/kr_us_strategy_map.md "entrySignal RS 배지 제거" 절 참고.
        [1] 배지·정렬 우선순위 둘 다 제거 — `immediateOrdered`의
        entrySignal 기반 aOk/bOk 비교자를 US 눌림목(us_pullback) 한정
        손절폭(진입가 대비 %) 오름차순 비교자로 교체(다른 소스는
        비교자가 0 반환 — 안정 정렬이라 원래 상대 순서 보존). 카드의
        entrySigBadge(개별 신호등 표시) span 완전 제거.
        [2][측정] RS/피벗 대비 거리(entry/pivot 비율)/20일선 이격 3개
        후보가 EV와 상관 있는지 90개 체크포인트로 측정 시작(백그라운드)
        — `scripts/measurements/2026-09-08_us_pullback_immediate_2nd_
        sort_candidates.py`. harness.py 재사용(새 로직 없음), US 전용
        5년 fetch, 사전 등록 기준(베스트-워스트 EV 격차≥0.15R & z≥1.96
        & 초반/후반 반분 재현) 통과 후보만 채택 — 결과는 별도 보고.
        [3] "🔴 즉시 행동" 상위 5건 + "나머지 N건 펼치기" — 2026-09-08
        실측(오늘 103건, 6배표본 90체크포인트 평균 ~107건/일)으로 US
        눌림목 히트가 일상적으로 40~100+건임을 확인, 12건은 오히려
        예외적 저치였지만 정상 범위 자체가 이미 한 화면을 넘어 접기가
        필요하다는 결론은 유지. IMMEDIATE_COLLAPSE_N=5 상수 도입,
        `_immediateExpanded` 전역 상태 + `toggleImmediateExpanded()`
        (다른 접힘류와 같은 패턴 — 전역 상태 + renderCalendar 전체
        재렌더). N=5 근거: 게이트 🟡(조정 압박) 상한 max_open_r=1.5R
        기준 하루 진입 가능 건수(1~2건)의 여유분. 정렬 기준은 [2]
        측정 결과가 나오면 그걸로, 아직은 [1]의 손절폭 오름차순 그대로.
        검증: 위 조사에 쓴 실측 스크립트로 today_decision.immediate를
        mock으로 직접 호출해 (a) entrySignal 배지 텍스트 미노출 (b)
        7건 입력 시 손절폭 오름차순으로 상위5만 노출+"나머지 2건
        펼치기" (c) 펼치기 클릭 시 7건 전부+"접기"로 전환 (d) 다시
        접기 클릭 시 상위5로 복귀(펼친 상태에서 hiddenCount=0이 되며
        토글 줄 자체가 사라져 못 되돌리던 버그를 구현 중 자체 발견·
        수정 — 조건을 hiddenCount>0에서 전체건수>N으로 변경) 전부 확인.
v5.212 [UI 개선 + 정밀화] v5.211 후속(사용자 지시 — "손절 폭을 ATR
        배수로 표기"). v5.211에서 이미 구현한 [1](시나리오 ①②에 ATR
        배수 병기)·[2](1.0ATR 미만 경고+★ 자격에 AND 조건)는 그대로
        두고 2건 추가 + 문서 정밀화.
        [3] 🔴 즉시행동 카드의 "진입 X · 손절 Y (-2.4%)" 배지에도 ATR
        배수 병기 — todayDecisionRiskBadge()가 이미 갖고 있던
        item.atr_pct로 atrMult = riskPct/atr_pct 추가 계산(새 필드 없음),
        "(-2.4%, 1.2ATR)" 형식으로 통일. 이 함수는 근접(🟡) "뚫으면
        진입" 배지와도 공유돼 자동으로 같이 적용됨.
        [2][문구 정정] "⚠️ 노이즈 손절 위험 — 손절 도달률 높음"→"⚠️
        노이즈 손절 — 도달률 높음"(사용자 지시 문구로 교체, v5.211
        renderScenarioHtml의 noiseWarn() 한 곳만 수정 — 판정 로직 무변경).
        [4] docs/kr_us_strategy_map.md "손절폭(ATR 배수)과 손절 도달률의
        관계" 절의 "주의" 문단을 사용자 지시대로 정밀화 — v5.211에서는
        "저ATR 버킷 표본 부족"이라고만 뭉뚱그렸는데, 실제로는 손절
        정의가 갈리는 지점이 핵심: 눌림목/박스돌파/돌파/추세전환(4탭)은
        운영 손절이 "구조손절+ATR버퍼"라 1.0~1.5ATR 미만 거리 자체가
        구조적으로 드물어(baseline n=2~55, 전부 100 미만) 저ATR 구간
        검정이 원천적으로 불가능(판정불가) — 단조감소로 보이는 숫자는
        실재하지만 통계적으로 뒷받침 못 함. 돌파임박만 손절 정의가
        "signal_low"라 저ATR 표본이 충분(1.0~1.5 baseline KR n=261/
        US n=310, 유일하게 100+)해 검정 가능한데, 그 결과(1.0~1.5 대비
        상한검정)는 KR/US 둘 다 전 구간 |z|<1.31로 비유의(KR 1.5~2.0
        z=-0.06/2.0~3.0 z=-0.33/3.0+ z=-0.74, US 1.5~2.0 z=1.30/
        2.0~3.0 z=0.77/3.0+ z=0.80) — 유일하게 검정 가능한 탭에서조차
        "넓힐수록 유의하게 나빠지거나 좋아진다"는 상한 근거가 없다.
        결론: "손절 폭은 %가 아니라 ATR 배수로 평가해야 한다"는
        방향성은 10개 탭×시장 원자료 전부 일관되게 뒷받침되지만, "몇
        ATR 미만은 나쁘다"는 구체적 임계값(1.0ATR 등)은 검증된 게
        아니라 체감상 참고 기준선 — 그래서 게이트가 아니라 표시
        전용으로만 반영한다는 점을 문서에 명시.
        검증: todayDecisionRiskBadge({entry:35.60, stop:34.76,
        atr_pct:2.0}) → "(-2.4%, 1.2ATR)" 실제 사용자 제시 예시값으로
        재현 확인. renderScenarioHtml Node 추출 테스트로 새 노이즈
        경고 문구 확인. 돌파임박 상한검정 z값은
        scripts/measurements/2026-09-07_kr_us_confirm_entry_stop_width_atr_multiple_ev.results.json
        에서 직접 재추출해 문서 수치와 대조.
...
v5.209 [UI 개선] 섹터 흐름 카드 압축(사용자 지시, static/index.html만
        ...
        60일|RS↕|대장(3))로 교체. 숫자 열 고정폭·우측정렬(colgroup),
        섹터명 12자 초과 시 "…" 말줄임(title에 전체명). KR·US를
        grid-template-columns 2열로 나란히 배치하고, 캘린더 상단 시장
        필터가 한국/미국이면 해당 열만 1fr 전폭. 대장은 KR 6자로 자르고
        US는 티커 그대로("PBF · VLO · DK"), qualifies=false(200일선/
        EPS 게이트 미달)는 회색+⚠️+툴팁 그대로 유지, 각 이름은 여전히
        트레이딩뷰 링크(tvUrl 재사용). 가속 블록도 같은 테이블 구조로
        통일 — RS 열만 "34→96"(60일RS→20일RS) 형식으로 교체, 20일/60일
        열은 그대로. 52주 신고가는 칼럼 헤더 옆 "🔥N" 배지(title에 섹터
        목록)로 압축. 기본 접힘 = 상위3+가속3만, "▾ 더보기(5개)"로
        5개까지 확장(전역 상태 + renderCalendar 재렌더, 기존 토글류와
        같은 패턴) — 접힌 상태 기준 대략 12줄 안팎으로 계산돼(헤더+열당
        헤더/표헤더/3행+가속헤더/표헤더/최대3행+더보기 1줄) 화면 1/3
        목표에 부합할 것으로 예상(실제 브라우저 픽셀 측정은 안 함,
        구조·로직 확인 수준).
        검증: 함수만 추출해 Node로 직접 렌더 — market=all(2열)/kr(1열
        KR 전폭) 둘 다 확인, 12자 초과 섹터명 말줄임("Oil & Gas
        Refining"→"Oil & Gas Re…") · 6자 초과 KR 대장명 절단("에이플러스
        에셋"→"에이플러스에") · qualifies=false 회색·⚠️ 표시 · 가속 행
        "40→99" 형식 · 🔥 배지 · "더보기" 노출 조건(5개 초과 시에만)
        전부 정상 출력 확인.
        hits_today(5탭 히트수) 필드는 이번 압축 레이아웃에 자리가 없어
        화면 표시에서 뺐음(백엔드 계산 자체는 유지, 필요하면 나중에
        다른 자리에 다시 노출 가능) — 지시에 없던 항목이라 명시.
...
def _jongga_session_state(now_kst: datetime) -> dict:
    ...
    안내 문구용(사용자 지시 4번). 평일 09:00~14:40=대기, 14:40~20:00=활성,
    그 외=장종료. 주말은 항상 장종료로 취급.

    v5.264(사용자 확정 — KRX 공식 종가 = 애프터마켓 포함): 활성 창 끝을
    **15:30 → 20:00**으로, 문구를 "15:20 동시호가 전" → "19:50~20:00
    애프터마켓"으로 바꿨다. 백테스트의 "T일 종가 매수"가 가리키는 종가는
    **20:00 확정 종가**이고(삼성전자 09-14 248,500으로 확인), 15:20 동시호가
    가격은 **정규장 마지막 가격일 뿐 종가가 아니다** — 그 시각에 사면
    백테스트와 다른 가격에 진입하게 된다.
    후보 선정 시점(14:40~15:00 스냅샷)은 그대로다 — 매수 시각만 바뀐다."""
    wd = now_kst.weekday()
    ...
    if hm < 20 * 60:
        return {"state": "active", "label": "⏰ 오늘 19:50~20:00 애프터마켓 진입용"}
    return {"state": "after", "label": "오늘 장 종료 — 내일 후보는 14:40에"}
...
# v5.264 재검토(사용자 요청): 진입 시각이 19:50~20:00으로 바뀌었지만 **이 값은
# 15:20 그대로 둔다.** 이 문턱의 의미는 "진입 시각"이 아니라 **"후보가 나왔는가"**다
# — 스케줄러 스냅샷이 14:40~15:00에 후보를 만들고, 그 뒤엔 화면에 후보가 뜨므로
# 이 안내(즉시행동 0건 사유)를 띄울 이유가 없어진다. 19:50으로 올리면 15:00~19:50
# 사이에 **후보가 화면에 있는데도 "아직 없다"는 안내가 뜨는** 모순이 생긴다.
# 대신 문구를 "종가베팅 **후보는** 15:20 이후"로 고쳐 진입 시각으로 오해되지 않게 했다.
JONGGA_READY_HM = 15 * 60 + 20     # KST 15:20 — 후보 등장 시각(진입 시각 아님)
    ...
    if not immediate:
        ...
        if _session_mkt_upper == "KR":
            ...
            if not jongga_today and _now_hm < JONGGA_READY_HM:
                _reason_parts.append(f"종가베팅 후보는 {JONGGA_READY_LABEL} 이후")
